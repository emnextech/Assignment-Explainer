    ...
    <AppShell onSignOut={() => void signOut()}>
      <div className="mb-6 max-w-4xl">
        ...
        <p className="mt-4 text-sm leading-8 text-ink/65">
          The goal is to help a student understand the assignment faster, prepare research properly, and avoid wasting time on the wrong structure or weak interpretation.
        </p>
      </div>
      <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        ...
        <aside className="space-y-4">
          <div className="rounded-[32px] border border-ink/8 bg-white p-6 shadow-soft">
            <p className="text-sm uppercase tracking-[0.24em] text-ink/45">Best results come from</p>
            <ul className="mt-5 space-y-4 text-sm leading-7 text-ink/75">
              ...
              <li>Leaving the command words exactly as written: discuss, evaluate, compare, analyse, justify.</li>
            </ul>
          </div>
          <div className="rounded-[32px] border border-ink/8 bg-white p-6 shadow-soft">
            <p className="text-sm uppercase tracking-[0.24em] text-ink/45">What the result should help with</p>
            <div className="mt-5 grid gap-3">
              {[
                ...
                <div key={item} className="rounded-[22px] border border-ink/6 bg-sand px-4 py-3 text-sm leading-7 text-ink/75">
                  {item}
                </div>
              ))}
            </div>
